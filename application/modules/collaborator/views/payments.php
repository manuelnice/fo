<section id="content"> <section class="vbox"> <section class="scrollable padder">
	...
	<header class="panel-heading"> All Transactions done by Finance Department</header>
	<div class="row text-sm wrapper">
		<div class="col-sm-5 m-b-xs">
			...
			<a href="<?=base_url()?>finance/add_fee" class="btn btn-sm btn-danger"><i class="fa fa-plus"></i> New Payment</a>
		</div>
	...
	<div class="table-responsive">
		<table class="table table-striped b-t b-light text-sm">
			<thead>
				...
				<?php
								if (!empty($payments)) {
				foreach ($payments as $key => $t) { ?>
				<tr>
					<td><?=$t->txn_id?></td>
					<td><?=$this->user_profile->get_profile_details($t->student,'reg_no')?></td>
					<td><?=$this->config->item('currency')?> <?=number_format($t->amount)?></td>
					<td><?=$t->bank_name?> </td>
					<td><a href="<?=base_url()?>finance/payments/view_txn/<?=$t->fee_id?>" class="btn btn-info btn-xs"><i class="fa fa-ticket"></i> View Transaction</a></td>
				</tr>
				<?php  }} else{ ?>
				<tr>
