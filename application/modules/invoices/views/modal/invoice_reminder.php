<div class="modal-dialog">
	...
	foreach ($invoice_details as $key => $invoice) { ?>
		...
		<h4 class="modal-title"><?=lang('invoice_reminder')?></h4>
          ...
          echo form_open(base_url().'invoices/manage/reminder',$attributes); ?>
          <?php
		...
		<div class="modal-body">
			...
			<input type="hidden" name="amount" value="<?=number_format(($invoice_cost + $tax) - $payment_made,2)?>">
				...
				<div class="col-lg-8">
			<textarea name="message" class="form-control" style="height:280px;"><?=$this->config->item('reminder_message')?></textarea>
				</div>
				</div>
			
		</div>
		<div class="modal-footer"> <a href="#" class="btn btn-default" data-dismiss="modal"><?=lang('close')?></a> 
		<button type="submit" class="btn btn-info"><?=lang('send_reminder')?></button>
		</form>
		</div>
	</div>
	<?php } ?>
