...
class Classmates extends MX_Controller {
	...
	$data['page'] = 'classmates';
	$data['classmates'] = $this->profile_model->get_all_records($table = 'account_details',
		$array = array('year_of_study' => $this->user_profile->get_profile_details($this->tank_auth->get_user_id(),'year_of_study'), 
		...
		$join_table = 'users',$join_criteria = 'users.id = account_details.user_id');
	$this->template
	->set_layout('users')
	->build('classmates',isset($data) ? $data : NULL);
	}
	function activity()
	{
	$this->load->module('layouts');
