<section id="content">
	<section class="vbox">
		...
		<div class="wrapper">
			...
			?>
			<small class="text-uc text-xs text-muted">Study Details</small>
				<p> <span class="label label-info">
				Year: <?=$this->user_profile->get_profile_details($this->tank_auth->get_user_id(),'year_of_study')?> Sem: <?=$this->user_profile->get_profile_details($this->tank_auth->get_user_id(),'semester')?></span></p> 
		...
		<section class="vbox">
			...
			<?php  echo modules::run('sidebar/flash_msg');?>
				<div class="wrapper"> <section class="panel panel-default">
				 ...
				 <?php     
				echo form_open(base_url().'profile/update_status'); ?>
					<input type="hidden" name="r_url" value="<?=uri_string()?>">
						<textarea name="status_update" class="form-control no-border" rows="3" placeholder="What's in your mind?..."></textarea>
					<footer class="panel-footer bg-light lter"> <button class="btn btn-info pull-right btn-sm" type="submit">POST</button>
					</form> 
					<ul class="nav nav-pills nav-sm">
						<li><a href="#"><i class="fa fa-camera text-muted"></i></a></li>
					</ul> </footer> </section> <section class="panel panel-default"> <h4 class="font-thin padder">Your Latest Posts</h4>
					<ul class="list-group">
					<?php
								if (!empty($updates)) {
						...
						<p><?=$u->post?> </p> 
						<small class="block text-muted"><i class="fa fa-clock-o"></i> <?php
								$today = time();
								$posted_time = strtotime($u->date_posted) ;
					...
					<div class="panel-body"> <a href="#" class="thumb pull-left m-r"> <img src="<?=IMG_URL?><?=$this->user_profile->get_profile_details($this->tank_auth->get_user_id(),'avatar')?>" class="img-circle"> </a>
						<div class="clear"> <a href="#" class="text-info">@wmandai <i class="fa fa-user"></i></a> <small class="block text-muted"><?php $this->db->where('posted_by',$this->tank_auth->get_user_id());
							$query = $this->db->get('posts');
							if ($query->num_rows() > 0){
								echo $query->num_rows();
							}else{
						...
						?> posts / <?php $this->db->where('liked_by',$this->tank_auth->get_user_id());
							$query = $this->db->get('likes');
							if ($query->num_rows() > 0){
								echo $query->num_rows();
							}else{
						...
						?> likes</small> <a href="<?=base_url()?>profile" class="btn btn-xs btn-success m-t-xs">Account</a>
						</div>
					</div> </section>
