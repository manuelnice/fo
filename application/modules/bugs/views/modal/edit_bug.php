<div class="modal-dialog">
	<div class="modal-content">
		...
		<h4 class="modal-title"><?=lang('edit')?> <?=lang('bugs')?></h4>
          ...
          echo form_open(base_url().'bugs/view/edit',$attributes); ?>
          <?php
		...
		<div class="modal-body">
			<p><?=lang('report_bug_message')?></p>
			 <input type="hidden" name="bug_id" value="<?=$bug->bug_id?>">
				...
				<option value="<?=$bug->reporter?>">Use Current</option>
					<?php
					if (!empty($users)) {
		...
		<button type="submit" class="btn btn-info"><?=lang('report_issue')?></button>
		</form>
		<?php }} ?>
		</div>
	</div>
