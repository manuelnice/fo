...
class Auth extends MX_Controller
{
	...
	$this->template->title('Welcome to '.$this->config->item('company_name'));
			...
			if ($this->form_validation->run($this)) {								// validation ok
				if (!is_null($data = $this->tank_auth->create_user(
					...
					if ($email_activation) {									// send "activate" email
						...
						unset($data['password']); // Clear password (just for any case)
			$this->session->set_flashdata('message', '<strong>You have Registered Successfully!</strong> We are thrilled to have you on board.');
						redirect('/auth/login');
