<header class="bg-black header navbar navbar-fixed-top-xs">
	<div class="navbar-header aside-md"> <a class="btn btn-link visible-xs" data-toggle="class:nav-off-screen" data-target="#nav"> <i class="fa fa-bars"></i> </a> <a href="#" class="navbar-brand" data-toggle="fullscreen"><img src="<?=IMG_URL?><?=$this->config->item('company_logo')?>" class="m-r-sm"><?=$this->config->item('company_name')?></a> <small>v <?=$this->config->item('version')?></small> <a class="btn btn-link visible-xs" data-toggle="dropdown" data-target=".nav-user"> <i class="fa fa-cog"></i> </a>
	</div>
			...
			<span class="font-bold"></span>
				...
				<div class="wrapper lter m-t-n-xs"> <a href="#" class="thumb pull-left m-r"> <img src="<?=AVATAR_URL?><?=$this->user_profile->get_profile_details($this->tank_auth->get_user_id(),'avatar')?>" class="img-circle"> </a>
					...
					<a href="<?=base_url()?>profile/settings">
						...
						</span> <small class="block"><?=lang('role')?>: <?=ucfirst($this->tank_auth->user_role($this->tank_auth->get_role_id()))?></small> <a href="<?=base_url()?>profile/settings" class="btn btn-xs btn-success m-t-xs">My Account</a>
					</div>
				</div>
			</section> </li>
			</ul>
